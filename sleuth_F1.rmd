...
To do this, we have to use `read_kallisto_hdf5()` and `write_kallisto_hdf5()` to subset our data. 


```{r}
source("subset_kallisto.R")
load("phasedData.Rdata")
geneIDs <- as.character(unique(phasedData$GeneID))

sample_id <- dir(file.path("results"))

kal_subset_dirs <- file.path("results_subset", sample_id, "kallisto")
kal_subset_dirs
...
LoadModifyWrite <- function(kal_dir) {
  ...
  obj <- subset_kallisto(obj, target_ids = geneIDs)
  write_kallisto_hdf5(kal = obj, fname = file.path(kal_dir, "abundance.h5"))
}

lapply(kal_subset_dirs, LoadModifyWrite)
```
