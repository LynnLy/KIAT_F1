...
sleuth_save(so, file = "results/sleuthOutput.Rdata")
#save(metadata, file = "metadata.Rdata")
sleuth_load("results/SleuthOutput.Rdata")
#load("metadata.Rdata")
```
...
plot_bootstrap(so, "BnaA02g22240D", units = "est_counts", color_by = "cultivar")
```
...
```{r}
plot_pca(so, color_by = 'cultivar', units = "est_counts")
...
## Removing Outlier BnaC09g16500D

```{r}
metadata <- dplyr::filter(metadata, sample != "BnaC09g16500D")
so <- sleuth_prep(metadata, extra_bootstrap_summary = TRUE)
