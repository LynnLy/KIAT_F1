...
ExtractASE <- function(MBASEDSummary, samples = 1, p = 0.05, MAFD = 0.2) {
  ...
  adjustedP <- p.adjust(MBASEDSummary$geneOutput$pValueASE, method = "BH", n = length(MBASEDSummary$geneOutput$pValueASE))
...
significant <- final414vs415 %>% SummarizeASEResults_2s() %>% ExtractASE()
significantRev <- final415vs414 %>% SummarizeASEResults_2s() %>% ExtractASE()
significantb <- final414vs415b %>% SummarizeASEResults_2s() %>% ExtractASE()
significantRevb <- final415vs414b %>% SummarizeASEResults_2s() %>% ExtractASE()
